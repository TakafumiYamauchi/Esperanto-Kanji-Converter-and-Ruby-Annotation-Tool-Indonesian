...
#=====================================================================
st.set_page_config(
    page_title="Alat Pembuatan Berkas JSON untuk Penggantian Teks (Karakter Han) dalam Teks Esperanto",
    layout="wide"
)
st.title("Membuat Berkas JSON untuk Menggantikan (Karakter Han) dalam Teks Esperanto")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方)
#=====================================================================
with st.expander("Tampilkan penjelasan cara penggunaan", expanded=True):
    st.markdown("""
    #### Pendahuluan
    Pada halaman ini, Anda dapat membuat dan mengunduh berkas JSON
    (yang dapat mencapai ukuran sekitar 50MB)
    untuk digunakan dalam proses penggantian teks Esperanto
    di halaman utama (main page).

    **Langkah penggunaan**:
    1. Unggah atau gunakan default **berkas CSV** yang berisi pasangan
       (akar kata Esperanto → terjemahan bahasa Indonesia) atau data lain yang diperlukan.
    2. Jika perlu, unggah atau gunakan default **berkas JSON** 
       (aturan pemecahan akar kata dan/atau pengaturan teks pengganti).
    3. Klik “Buat Berkas JSON untuk Penggantian” untuk menghasilkan
       **berkas JSON** dan kemudian unduh hasilnya.

    Di bawah ini disediakan juga beberapa contoh berkas,
    yang dapat dijadikan referensi format atau kustomisasi lebih lanjut.
    """)
...
# ダウンロードするボタンを設けています。
#=====================================================================
with st.expander("Daftar Berkas Contoh (Untuk Diunduh)"):
    st.write("#### Daftar Berkas Contoh")

    # サンプルCSV１
    st.markdown("""
    **Contoh CSV 1 (Daftar akar kata Esperanto – terjemahan bahasa Indonesia + anotasi ruby)**  
    File CSV yang memetakan satu per satu akar kata Esperanto ke terjemahan bahasa Indonesia.  
    Dengan menyesuaikan format CSV seperti ini, lalu mengunggahnya, Anda dapat membuat berkas JSON untuk penggantian teks.
    """)
    file_path0 = './Appの运行に使用する各类文件/Daftar akar kata Esperanto dengan terjemahan bahasa Indonesia dan anotasi ruby.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Unduh Contoh CSV 1 (Akar Kata Esperanto – Terjemahan Indonesia + anotasi ruby)",
            data=file,
            file_name="AkarKataEsperanto_TerjemahanIndonesia_AnotasiRuby.csv",
            mime="text/csv"
        )

    # サンプルCSV２
    st.markdown("""
    **Contoh CSV 2 (Akar kata Esperanto – Daftar Hanzi, proposal Mingeo di Zhihu)**  
    Ini adalah CSV yang memetakan akar kata Esperanto ke karakter Han (Hanzi).
    """)
    file_path0 = './Appの运行に使用する各类文件/Mingeo先生版 世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Unduh Contoh CSV 2 (Akar kata Esperanto – Hanzi versi Mingeo)",
            data=file,
            file_name="AkarKataEsperanto_Hanzi_Mingeo.csv",
            mime="text/csv"
        )

    # サンプルCSV３
    st.markdown("""
    **Contoh CSV 3 (Akar kata Esperanto – Daftar Hanzi)**  
    Ini adalah CSV yang memetakan akar kata Esperanto ke karakter Han.
    """)
    file_path0 = './Appの运行に使用する各类文件/世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Unduh Contoh CSV 3 (Akar kata Esperanto – Hanzi)",
            data=file,
            file_name="AkarKataEsperanto_Hanzi.csv",
            mime="text/csv"
        )

    # サンプルJSON１
    st.markdown("""
    **Contoh JSON 1 (Pengaturan Pemecahan Akar Kata Esperanto oleh Pengguna)**  
    **Tujuan**: Mengatur cara memecah akar kata Esperanto,
    menambahkan akhiran (mis. akhiran kata kerja) dalam bentuk turunan tertentu,
    dan mengatur kapan penggantian tersebut dimasukkan.
    Silakan lihat komentar di dalam berkas contoh.  
    (Misal: `["am", "dflt", ["verbo_s1"]]` dst.)
    """)
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Unduh Contoh JSON 1 (Pengaturan Pemecahan Akar Kata Esperanto)",
            data=file_json,
            file_name="PengaturanPemecahanAkarKataEsperanto.json",
            mime="application/json"
        )

    # サンプルJSON２
    st.markdown("""
    **Contoh JSON 2 (Pengaturan Pengguna untuk Teks Pengganti)**  
    **Tujuan**: Jika ada kata tertentu yang membutuhkan karakter Han khusus atau format unik,
    Anda dapat menambahkannya di sini, di luar pemecahan akar kata di berkas JSON sebelumnya.
    (Secara umum, Anda cukup mengedit berkas CSV atau mengatur pemecahan akar kata dengan JSON 1.)
    """)
    json_file_path2 = './Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json'
    with open(json_file_path2, "rb") as file_json:
        btn_json = st.download_button(
            label="Unduh Contoh JSON 2 (Pengaturan Teks Pengganti)",
            data=file_json,
            file_name="PengaturanTeksPengganti.json",
            mime="application/json"
        )
    st.markdown("""
    **Contoh Excel 1 (Daftar 14 bahasa: akar kata Esperanto + anotasi terjemahan)**  
    Terdapat 14 bahasa (Jepang, Tiongkok, Korea, Inggris, Rusia, Spanyol, Italia, Prancis,
    Jerman, Arab, Hindi, Polandia, Vietnam, Indonesia)  
    beserta anotasi terjemahan untuk masing-masing akar kata Esperanto (4736 entries).
    """)
    with open('./Appの运行に使用する各类文件/Kreado de rubenaj komentoj en 14 lingvoj (日本語, 中文, 한국어, English, Русский, español, italiano, français, Deutsch, العربية, हिन्दी, polski, Tiếng Việt, Bahasa Indonesia) respondaj al listo de 4736 Esperant.xlsx', "rb") as file:
        st.download_button(
            label="Unduh Contoh Excel 1 (14 bahasa: akar kata Esperanto + anotasi terjemahan)",
            data=file,
            file_name="Daftar14Bahasa_AkarEsperanto_denganAnotasiTerjemahan.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

    # サンプルExcel２
    st.markdown("""
    **Contoh Excel 2 (Akar Kata Esperanto – Terjemahan Bahasa Jepang + anotasi (level pemahaman))**  
    **Tujuan**: Berguna jika Anda ingin menambahkan rubi atau anotasi
    terhadap akar kata Esperanto khusus untuk bahasa Jepang.
    Di dalamnya juga terdapat "tingkat penguasaan" berdasarkan kamus dasar Esperanto-Jepang.
    """)
    with open('./Appの运行に使用する各类文件/エスペラント語根-日本語訳ルビ対応リスト(習得レベル付き).xlsx', "rb") as file:
        st.download_button(
            label="Unduh Contoh Excel 2 (Akar Kata Esperanto – Terjemahan Jepang + level pemahaman)",
            data=file,
            file_name="AkarEsperanto_TerjemahanJepang_LevelPemahaman.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
...
#=====================================================================
options = {
    "HTML (dengan penyesuaian ukuran huruf rubi)": "HTML格式_Ruby文字_大小调整",
    "HTML (dengan penyesuaian ukuran huruf rubi + penggantian karakter Han)": "HTML格式_Ruby文字_大小调整_汉字替换",
    "HTML (dasar)": "HTML格式",
    "HTML (penggantian karakter Han)": "HTML格式_汉字替换",
    "Format tanda kurung": "括弧(号)格式",
    "Format tanda kurung (penggantian karakter Han)": "括弧(号)格式_汉字替换",
    "Penggantian sederhana": "替换后文字列のみ(仅)保留(简单替换)"
}


display_options = list(options.keys())
selected_display = st.selectbox('Pilih format output:', display_options)
format_type = options[selected_display]

# Contoh sederhana: main_text_list dan ruby_content_list dipetakan melalui loop
...
st.write("---")
st.markdown("**Teks yang sudah diformat ⇓**")
...
#=====================================================================
st.header("Langkah 1: Persiapkan Berkas CSV")
st.markdown(
    """
### Pilih **berkas CSV** yang berisi akar kata Esperanto dan informasi terjemahan (karakter Han) 

---
    """
)

csv_choice = st.radio("Bagaimana Anda ingin menggunakan berkas CSV?", ("Unggah Berkas CSV", "Gunakan Default"))
csv_path_default = "./Appの运行に使用する各类文件/Daftar akar kata Esperanto dengan terjemahan bahasa Indonesia dan anotasi ruby.csv"

CSV_data_imported = None

if csv_choice == "Unggah Berkas CSV":
    st.write("Silakan unggah berkas CSV (disarankan UTF-8).")
    ...
    if uploaded_file is not None:
        ...
        st.success("Berkas CSV berhasil diunggah.")
    else:
        st.warning("Tidak ada berkas CSV yang diunggah.")
        st.stop()

elif csv_choice == "Gunakan Default":
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("Menggunakan berkas CSV bawaan (default).")
    except FileNotFoundError:
        st.error("Berkas CSV default tidak ditemukan. Proses dibatalkan.")
        st.stop()

st.write("Berkas CSV berhasil dibaca. Lanjutkan ke langkah berikutnya.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備
#=====================================================================
st.header("Langkah 2: Persiapkan Berkas JSON (aturan pemecahan akar kata, dsb.)")
st.markdown("""
Anda dapat mengunggah atau menggunakan berkas JSON bawaan yang berisi
**aturan pemecahan akar kata Esperanto** dan/atau
**teks pengganti khusus**.

Silakan unduh salah satu **Contoh JSON** dan modifikasi sesuai kebutuhan,
lalu unggah kembali.
""")

json_choice = st.radio("1. Bagaimana Anda menangani berkas JSON untuk mendefinisikan pemecahan akar kata Esperanto?", ("Unggah Berkas JSON", "Gunakan Default"))
json_path_default = "./Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json"
custom_stemming_setting_list = None

if json_choice == "Unggah Berkas JSON":
    uploaded_json = st.file_uploader("Unggah berkas JSON", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("Berkas JSON berhasil diunggah.")
    else:
        st.warning("Tidak ada berkas JSON yang diunggah.")
        st.stop()
elif json_choice == "Gunakan Default":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("Menggunakan berkas JSON bawaan (default).")
    except FileNotFoundError:
        st.error("Berkas JSON default tidak ditemukan.")
        st.stop()

json_choice2 = st.radio("2. Bagaimana Anda menangani berkas JSON untuk teks pengganti?", ("Unggah Berkas JSON", "Gunakan Default"))
json_path_default2 = "./Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json"
user_replacement_item_setting_list = None

if json_choice2 == "Unggah Berkas JSON":
    uploaded_json = st.file_uploader("Unggah berkas JSON kedua", type=['json'])
    if uploaded_json is not None:
        ...
        st.warning("Tidak ada berkas JSON yang diunggah.")
        st.stop()
elif json_choice2 == "Gunakan Default":
    try:
        with open(json_path_default2, "r", encoding="utf-8") as g:
            user_replacement_item_setting_list = json.load(g)
        st.info("Menggunakan berkas JSON kedua bawaan (default).")
    except FileNotFoundError:
        st.error("Berkas JSON kedua default tidak ditemukan.")
        st.stop()
...
# ステップ3: 高度な設定 (並列処理)
#=====================================================================
st.header("Langkah 3: Pengaturan Lanjutan (Pemrosesan Paralel)")
with st.expander("Buka pengaturan terkait pemrosesan paralel"):
    st.write("""
    Di sini Anda dapat menentukan jumlah proses yang digunakan
    untuk pemrosesan paralel saat pembuatan berkas JSON.
    Jika jumlah data (teks dan akar kata) sangat besar,
    pemanfaatan beberapa core CPU dapat meningkatkan kecepatan.
    """)

    use_parallel = st.checkbox("Gunakan pemrosesan paralel", value=False)
    num_processes = st.number_input("Jumlah proses paralel", min_value=2, max_value=6, value=5, step=1)

st.write("### Membuat Berkas JSON Penggantian Final (Tombol)")

#=====================================================================
# 置換用JSONファイルを作成するボタン
#=====================================================================
if st.button("Buat Berkas JSON untuk Penggantian"):
    with st.spinner("Sedang membuat berkas JSON untuk penggantian... Mohon tunggu sebentar."):
        # (中略はコード内部のため原文のまま)
        ...
        #     「エスペラント語根 → (汉字 or インドネシア語訳ルビ等)」を反映させる
        # -------------------------------------------------------------
        ...
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"{current_count}/{total_items} baris sedang diproses...")

            progress_bar.progress(100)
            progress_text.write("Proses paling memakan waktu telah mencapai 100%. (Tunggu 3~4 detik lagi...)")
        ...
        st.success("Pembuatan daftar penggantian selesai!")

        st.download_button(
            label="Unduh Daftar Penggantian Final (gabungan 3 berkas JSON)",
            data=download_data,
            file_name="DaftarPenggantian_Final_Gabungan3BerkasJSON.json",
            mime='application/json'
        )

...
#=================================================================
st.set_page_config(
    page_title="Alat untuk Mengganti Karakter (Kanji) dalam Teks Esperanto",
    layout="wide"
)

# タイトル部分（GUI表示のみインドネシア語に）
st.title("Mengganti Teks Esperanto dengan Karakter Kanji atau Menambahkan Anotasi HTML (Versi Diperluas)")
st.write("---")
...
selected_option = st.radio(
    "Bagaimana Anda ingin menangani berkas JSON? (Memuat berkas JSON pengganti)",
    json_options,
    format_func=lambda x: "Gunakan JSON bawaan (default)" if x == "デフォルトを使用する" else "Unggah berkas"
)

with st.expander("Unduh Contoh Berkas JSON (untuk penggantian)"):
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Unduh Contoh Berkas JSON",
            data=file_json,
            file_name="contoh_berkas_JSON.json",
...
if selected_option == "デフォルトを使用する":
    ...
    uploaded_file = st.file_uploader(
        "Unggah berkas JSON (format gabungan 3 daftar).json",
        type="json"
    )
    if uploaded_file is not None:
        try:
            ...
            st.success("Berkas JSON yang diunggah berhasil dimuat.")
        except Exception as e:
            st.error(f"Gagal membaca berkas JSON yang diunggah: {e}")
            st.stop()
    else:
        st.warning("Tidak ada berkas JSON yang diunggah. Proses dihentikan.")
        st.stop()
...
# 同時プロセス数の選択
#=================================================================
st.header("Pengaturan Lanjutan (Pemrosesan Paralel)")
with st.expander("Buka pengaturan untuk pemrosesan paralel"):
    st.write("""
    Di sini Anda dapat menentukan jumlah proses yang akan dijalankan secara paralel
    selama penggantian karakter (kanji).
    """)
    use_parallel = st.checkbox("Gunakan pemrosesan paralel", value=False)
    num_processes = st.number_input(
        "Jumlah proses yang berjalan bersamaan",
...
options_indonesian_labels = {
    'HTML格式_Ruby文字_大小调整': "Format HTML dengan anotasi Ruby dan penyesuaian ukuran",
    'HTML格式_Ruby文字_大小调整_汉字替换': "Format HTML dengan anotasi Ruby, penyesuaian ukuran, dan penggantian karakter Kanji",
    'HTML格式': "Format HTML",
    'HTML格式_漢字替换': "Format HTML dengan penggantian karakter Kanji",
    '括弧(号)格式': "Format dengan tanda kurung",
    '括弧(号)格式_漢字替换': "Format dengan tanda kurung dan penggantian karakter Kanji",
    '替换后文字列のみ(仅)保留(简单替换)': "Hanya menyimpan teks yang telah digantikan (penggantian sederhana)"
}

display_options = list(options.keys())
selected_display = st.selectbox(
    "Pilih format keluaran (sesuai dengan yang didefinisikan di berkas JSON pengganti):",
    display_options,
    format_func=lambda key: options_indonesian_labels[key]
)
...
st.subheader("Sumber teks masukan")
source_option = st.radio(
    "Bagaimana Anda ingin menyediakan teks masukan?",
    source_options,
    format_func=lambda x: "Masukkan secara manual" if x == "手動入力" else "Unggah berkas"
)

uploaded_text = ""

if source_option == "ファイルアップロード":
    text_file = st.file_uploader("Unggah berkas teks (pengkodean UTF-8)", type=["txt", "csv", "md"])
    if text_file is not None:
        uploaded_text = text_file.read().decode("utf-8", errors="replace")
        st.info("Berkas teks berhasil diunggah.")
    else:
        st.warning("Tidak ada berkas teks yang diunggah. Gunakan masukan manual atau unggah berkas.")
...
with st.form(key='profile_form'):
    ...
    text0 = st.text_area(
        "Silakan masukkan teks Esperanto di sini",
        height=150,
        value=initial_text
    )

    st.markdown("""Jika Anda membungkus sebagian teks dengan simbol **%** 
    (misalnya: `%<teks hingga 50 karakter>%`),
    maka bagian tersebut **tidak akan digantikan** dan dibiarkan apa adanya di hasil akhir.""")

    st.markdown("""Demikian pula, jika Anda membungkus sebagian teks dengan simbol **@** 
    (misalnya: `@<teks hingga 18 karakter>@`),
    maka bagian tersebut akan digantikan secara **lokal** (terbatas pada bagian itu saja).""")

    letter_type = st.radio(
        'Pilih cara menampilkan karakter khusus Esperanto di hasil keluaran',
        ('上付き文字', 'x 形式', '^形式'),
        format_func=lambda x: (
            "Karakter aksen di atas (ĉ → c + ˆ)" if x == "上付き文字"
            else ("Format x (ĉ → cx)" if x == "x 形式" else "Format ^ (ĉ → c^)")
        )
    )

    submit_btn = st.form_submit_button('Kirim')
    cancel_btn = st.form_submit_button("Batal")

    if cancel_btn:
        st.warning("Dibatalkan.")
        st.stop()
...
if processed_text:
    ...
    if len(lines) > MAX_PREVIEW_LINES:
        ...
        preview_text = "\n".join(first_part) + "\n...\n" + "\n".join(last_part)
        st.warning(
            f"Teks terlalu panjang (total {len(lines)} baris). "
            "Hanya menampilkan pratinjau singkat (247 baris pertama dan 3 baris terakhir)."
        )
    else:
        preview_text = processed_text

    if "HTML" in format_type:
        tab1, tab2 = st.tabs(["Pratinjau HTML", "Hasil (Kode HTML)"])
        with tab1:
            components.html(preview_text, height=500, scrolling=True)
        with tab2:
            st.text_area("Kode HTML yang dihasilkan:", preview_text, height=300)
    else:
        tab3_list = st.tabs(["Teks Hasil"])
        with tab3_list[0]:
            st.text_area("Hasil:", preview_text, height=300)

    download_data = processed_text.encode('utf-8')
    st.download_button(
        label="Unduh Hasil",
        data=download_data,
        file_name="hasil_penggantian.html",
        mime="text/html"
    )
st.write("---")
